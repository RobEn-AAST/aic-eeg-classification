...
class MLP(nn.Module):  # Checked
    def __init__(self, dim, mlp_size, mlp_dropout=0.1):
        ...
        self.mlp = nn.Sequential(nn.Linear(dim, mlp_size), nn.GELU(), nn.Dropout(mlp_dropout), nn.Linear(mlp_size, dim), nn.Dropout(mlp_dropout))
...
class EncoderLayer(nn.Module):  # Checked
    def __init__(self, dim, num_heads, mlp_size, attn_dropout, mlp_dropout):
        ...
        self.msa = Attention(dim=dim, num_heads=num_heads, attn_dropout=attn_dropout)

        self.mlp = MLP(dim=dim, mlp_size=mlp_size, mlp_dropout=mlp_dropout)
...
class Tokenizer(nn.Module):  # Tokenizer can be used to capture spatial information
    def __init__(
        self, kernel_sizes, stride, padding, pooling_kernel_size, pooling_stride, pooling_padding, n_conv_layers, n_input_channels, n_output_channels, in_planes, activation, max_pool, conv_bias
    ):
        ...
        self.conv_layers = nn.Sequential(
            *[
                nn.Sequential(
                    nn.Conv2d(n_filter_list[i], n_filter_list[i + 1], kernel_size=kernel_sizes[i], stride=stride, padding=padding, bias=conv_bias),
                    nn.Identity() if activation is None else activation(),
                    nn.AvgPool2d(kernel_size=pooling_kernel_size, stride=pooling_stride, padding=pooling_padding) if max_pool else nn.Identity(),  # maxpool
                )
    ...
    def sequence_length(self, n_channels=3, height=224, width=224):  # MUST be changed
...
class Transformer(nn.Module):
    def __init__(self, dim, num_layers, num_heads, num_classes, attn_dropout, dropout, mlp_size, positional_embedding, sequence_length=None):
        ...
        assert sequence_length is not None or positional_embedding == "none", f"Positional embedding is set to {positional_embedding} and" f" the sequence length was not specified."
        ...
        self.blocks = nn.ModuleList([EncoderLayer(dim=dim, num_heads=num_heads, mlp_size=mlp_size, attn_dropout=attn_dropout, mlp_dropout=dropout) for i in range(num_layers)])
